...
conclusion `KERNEL_RECOVERY.md` reached about the recurrent kernel: the
...
`data/results.json`, `chorus_env.py` never reading `W`, and the
circular kernel-recovery in `KERNEL_RECOVERY.md`) raised the same
...
(confirmed in `KERNEL_RECOVERY.md`). The phase used here is the same
...
All on `data/cx_real.npz` (1,051 x 1,051 signed W) +
...
1. **The real-connectome pipeline that this repo can actually run
   ...
   a completely independent angle, what `KERNEL_RECOVERY.md` already
